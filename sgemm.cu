// This file is translated from:
// https://github.com/wangzyon/NVIDIA_SGEMM_PRACTICE/blob/master/sgemm.cu
...
int main(int argc, char **argv) {
    ...
    // Declare the handle and create the Handle.
    // cublasCreate will return a value of type cublasStatus_t to determine whether the handle is created successfully (value is 0)
    ...
    // Use cudaEvent for GPU Stream Timing.
    // cudaEvent is equivalent to publishing event tasks in the target stream
    float elapsed_time;
    cudaEvent_t beg, end;
    cudaEventCreate(&beg);
    cudaEventCreate(&end);
    ...
    for (int i = 0; i < size_len; i++) {
        ...
        printf("m=n=k=%d\n", m);
        // Verify the calculation correctness and pre-execute once before the kernel function timing to avoid cold start errors
        if (kernel_num != 0) {
            test_kernel(0, m, n, k, alpha, dA, dB, beta, dC_ref, handle);      // cuBLAS
            test_kernel(kernel_num, m, n, k, alpha, dA, dB, beta, dC, handle); // User defined
        ...
        elapsed_time /= 1000.;          // Conversion to seconds
